...
import { getCachedImage } from './image-compare';
...
export function analyzeDiffRegions(
  ...
  const diff = getCachedImage(diffImagePath);
  ...
  const regionDiffs: RegionDiff[] = regions.map((region, idx) => {
  ...
  if (maxPercentage > 20) overallSeverity = 'critical';
  else if (maxPercentage > 10) overallSeverity = 'high';
  else if (maxPercentage > 5) overallSeverity = 'medium';
  else if (maxPercentage > 0) overallSeverity = 'low';
 ...
 */
export function generateIssues(analysis: DiffAnalysis): VisualIssue[] {
  ...
  analysis.regionDiffs.forEach(region => {
    if (region.percentage > 10) {
      let category: 'layout' | 'size' | 'color' | 'spacing' | 'element' = 'layout';
      let description = '';
 ...
 */
export function generateTextualReport(
  testName: string,
  mismatchedPixels: number,
  totalPixels: number,
  ...
  if (issues.length > 0) {
    ...
    const categories = ['layout', 'size', 'color', 'spacing', 'element'] as const;
 ...
 * Save report to file
 */
export function saveReport(reportPath: string, reportContent: string): void {
